...
function Dataset() {
  ...
  const init = async () => {
    ...
    const dataPromises = res.data.map(async (v, i) => {
      let imageUrl;
      if (project.task_type === "pdfParser") {
  ...
  return (
    <Spin spinning={loading} tip={t('Dataset.Dataset.9034111-12')} indicator={<LoadingOutlined spin />} size="large">
      <MainBox>
        <Header>
          <Title>
            <h3>
            ...
            <Space size="middle">
              ...
              <Button ref={tourRef2} size="small" type="primary" onClick={parseDataset} disabled={!selectedFile.length}>
                {t('Dataset.Dataset.9034111-19')}
              </Button>
            </Space>
          </Title>
          <Search placeholder={t('Dataset.Dataset.9034111-20')} onSearch={() => { }} enterButton />
          <Actions></Actions>
        </Header>
...
const FileList = styled.div`
  height: 100%;
  display: grid;
  grid-template-columns: ${(p) => (p.$isNone ? "none" : "repeat(auto-fill, minmax(120px, 1fr))")};
...
const VideoCut = (props) => {
  return (
    <Form
      ...
      autoComplete="off"
    >
      <Form.Item label={t('Dataset.Dataset.9034111-21')} name="paramsType" rules={[{ required: true }]}>
        <Select
          options={[
      ...
      <Form.Item
        noStyle
        shouldUpdate={(prevValues, currentValues) => prevValues.paramsType !== currentValues.paramsType}
      >
        {({ getFieldValue }) => {
          const paramsType = getFieldValue("paramsType");
          return paramsType === "min_max" ? (
            <>
              <Form.Item label={t('Dataset.Dataset.9034111-24')} name="min" rules={[{ required: true }]}>
                <InputNumber style={{ width: "100%" }} placeholder={t('Dataset.Dataset.9034111-25')} />
              </Form.Item>
              <Form.Item label={t('Dataset.Dataset.9034111-26')} name="max" rules={[{ required: true }]}>
                <InputNumber style={{ width: "100%" }} placeholder={t('Dataset.Dataset.9034111-27')} />
              </Form.Item>
            </>
          ) : (
            <Form.Item label={t('Dataset.Dataset.9034111-23')} name="middle" rules={[{ required: true }]}>
              <InputNumber style={{ width: "100%" }} placeholder={t('Dataset.Dataset.9034111-28')} />
            </Form.Item>
          );
        }}
      </Form.Item>
