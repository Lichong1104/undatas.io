...
function VersionData(props) {
  ...
  const fileDetail = (file_id, result) => {
    if (!result) return message.warning('正在解析此文件！')
    const payload = {
      file_id,
  ...
  return (
    <MainBox>
      <Title>
        ...
        <span onClick={() => history.push("/versions/version-info")}>{t('VersionData.VersionData.903404-0')}</span>
      </Title>
      {loading ? (
        <div style={{ flex: 1 }}>
          <Spin spinning={loading} tip={t('VersionData.VersionData.903404-1')} indicator={<LoadingOutlined spin />} size="large">
            <div></div>
          </Spin>
        </div>
      ) : (
            ...
            return (
              <FileItem key={i} onClick={() => fileDetail(v.file_id, v.result)}>
                ...
                >
                  ...
                  > <Status>
                      {v.result ? <Badge status="success" /> : undefined}
                    </Status></ImageBox>
                </Badge.Ribbon>
                <p>{v.file_name}</p>
              </FileItem>
